...
export class PlaywrightService extends EventEmitter {
   ...
   */
  private async processUrlInBatch(
    url: string,
    limit: number,
    browserPool: any,
    options: any,
    ...
    if (!this.shouldProcessUrl(url)) return;
   ...
   */
  private async processDiscoveryResponse(
    url: string,
    response: any,
    baseUrl: string,
    includePaths: string[],
   ...
   */
  private async releasePageSafely(page: any, browserId: string, browserPool: any, url: string): Promise<void> {
    try {
